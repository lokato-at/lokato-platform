...
set_env_value() {
  ...
  ok "$key in $(basename "$file") gesetzt."
}
...
set_env_value "$BACKEND_DIR/.env" "API_SLOW_REQUEST_MS" "400"
set_env_value "$BACKEND_DIR/.env" "SSE_MAX_CONNECTION_SECONDS" "60"
set_env_value "$FRONTEND_DIR/.env" "VITE_API_BASE_URL" "http://${PUBLIC_HOST}:${BACKEND_PORT}/api/v1"
set_env_value "$FRONTEND_DIR/.env" "VITE_SSE_BASE_URL" "http://${PUBLIC_HOST}:${SSE_PORT}/api"
...
info "Starte Laravel REST API, SSE API, MQTT Subscriber und Frontend Preview..."
...
start_process "$RUN_DIR/backend-sse.pid" "$LOG_DIR/backend-sse.log" bash -lc "cd '$BACKEND_DIR' && php -d variables_order=GPCS artisan serve --host=0.0.0.0 --port=$SSE_PORT"
...
echo "Backend REST: http://${PUBLIC_HOST}:$BACKEND_PORT"
echo "Backend SSE:  http://${PUBLIC_HOST}:$SSE_PORT"
